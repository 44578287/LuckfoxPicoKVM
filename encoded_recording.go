...
func startEncodedRecording(filename string) (EncodedRecordingStatus, error) {
	...
	if filename == "" {
		filename = fmt.Sprintf("recording-%s%s", time.Now().Format("20060102-150405"), ext)
	}
	filename = filepath.Base(filename)
	if filename == "." || filename == string(filepath.Separator) || strings.Contains(filename, "..") {
		return getEncodedRecordingStatus(), fmt.Errorf("invalid recording filename")
	}
	if !strings.HasSuffix(strings.ToLower(filename), ext) {
		filename = strings.TrimSuffix(filename, filepath.Ext(filename)) + ext
	}

	if err := os.MkdirAll(imagesFolder, 0755); err != nil {
		return getEncodedRecordingStatus(), fmt.Errorf("create recording folder: %w", err)
	}
	fullPath := filepath.Join(imagesFolder, filename)
	file, err := os.OpenFile(fullPath, os.O_CREATE|os.O_EXCL|os.O_WRONLY, 0644)
	if err != nil {
		return getEncodedRecordingStatus(), fmt.Errorf("create recording: %w", err)
	}
	...
	enhancedRecording.running = true
	enhancedRecording.filename = filename
	enhancedRecording.path = fullPath
	enhancedRecording.codec = codec
	enhancedRecording.startedAt = time.Now()
	...
	for {
		...
		case frame, ok := <-frames:
			...
			if currentCodec != codec {
				frame.Release()
				setEncodedRecordingError(fmt.Sprintf("video codec changed from %s to %s; recording stopped", codec, currentCodec))
				return
			}
			...
			frame.Release()
			if err != nil {
				setEncodedRecordingError(fmt.Sprintf("write recording: %v", err))
				return
			}
			enhancedRecording.frames.Add(1)
			enhancedRecording.bytes.Add(uint64(n))

			// Storage check is intentionally sparse to keep the hot video path cheap.
			if enhancedRecording.frames.Load()%120 == 0 {
				if free, freeErr := userdataFreeBytes(); freeErr == nil && free < recordingReserveBytes {
					setEncodedRecordingError("recording stopped: free space reserve reached")
					return
				}
			}
		}
...
func finalizeEncodedRecording(subscriberID string, file *os.File, done chan struct{}) {
	...
	enhancedRecording.mu.Lock()
	if enhancedRecording.subscriberID == subscriberID {
...
func setEncodedRecordingError(message string) {
	enhancedRecording.mu.Lock()
	enhancedRecording.lastError = message
	enhancedRecording.mu.Unlock()
	logger.Warn().Str("error", message).Msg("encoded recording stopped")
}

func userdataFreeBytes() (uint64, error) {
