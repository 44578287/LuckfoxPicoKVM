...
export default function OtherSessionRoute() {
  ...
  const handleClose = () => {
    ...
    if (hasHealthyWebRTC) {
      // The video/data path already belongs to this tab. This modal can be a
      // delayed HTTP ownership event; rebuilding WebRTC here would kick our own
      // working session and can make the dialog recur.
      navigate("..");
      return;
    }

    // No healthy controller session remains. setupPeerConnection closes any
    // stale local PC before creating exactly one replacement session.
